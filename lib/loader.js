                    ...
                    if (pathMap[prefix] === false) {
                        ...
                        if (typeof pathMap[prefix].query === 'function') {
                            ...
                            if (classes instanceof Array) {
                                retVal = classes.map(className => className.src);
                            } else {
                                try {
                    ...
                    fileProps.requires.forEach((require) => {
                        ...
                        if (result instanceof Array) {
                            result.forEach((require) => {
                                requireStr += `require(${escodegen.generate({
                                    type: 'Literal',
                                    value: require.src
                                })});\r\n`
                            });
                            ...
                            requireStr += `require(${escodegen.generate({
                                type: 'Literal',
                                value: ExtParser.query(require).src
                            })});\r\n`
                        }
            ...
            esUtils.traverse(tree, function (node) {
                ...
                if (node && node.type === 'Property' && node.key && properties.includes(node.key.name)) {
                    ...
                    if (requireStr !== '') {
                        ...
                        if (configMap[nodeName].end === true) {
                            updates.push({type: 'add', start: parent.range[1], end: parent.range[1], data: requireStr});
                        } else {
